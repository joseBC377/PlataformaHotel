...
public class ReservaCompletaService {
        ...
        public Reserva crearReservaCompleta(ReservaCompletaRequest req) {
                ...
                for (DetalleHabitacionRequest d : req.getHabitaciones()) {
                        Habitacion habitacion = habitacionRepository.findById(d.getIdHabitacion())
                                        .orElseThrow(() -> new EntityNotFoundException(
                                                        "Habitación no encontrada: " + d.getIdHabitacion()));

                        if (habitacion.getEstado() != RolHabitacion.DISPONIBLE) {
                                throw new IllegalStateException(
                                                "La habitación " + habitacion.getNombre_habitacion()
                ...
                for (DetalleServicioRequest d : req.getServicios()) {
                        Servicio servicio = servicioRepository.findById(d.getIdServicio())
                                        .orElseThrow(() -> new EntityNotFoundException(
                                                        "Servicio no encontrado: " + d.getIdServicio()));

                        ReservaServicio rs = new ReservaServicio();
                        ReservaServicioId rsId = new ReservaServicioId(reserva.getId_reserva(),
                ...
                try {
                        DetalleHabitacionRequest primerDetalle = req.getHabitaciones().get(0);
                        Habitacion habitacionInfo = habitacionRepository.findById(primerDetalle.getIdHabitacion())
                                        .orElse(null);

                        if (habitacionInfo != null) {
                                emailService.enviarConfirmacionReserva(
                                                req.getEmailContacto(),
                                                usuario.getNombre_usuario() + " " + usuario.getApellido_paterno(),
                                                reserva.getId_reserva(),
                                                habitacionInfo.getNombre_habitacion(),
                                                habitacionInfo.getCategoriaHabitacion().getNombre_categoria(),
                                                primerDetalle.getFechaInicio(),
                                                primerDetalle.getFechaFin(),
                                                req.getPago().getTotal());
                        }
                } catch (Exception e) {
        ...
        public List<HistorialReservaResponse> obtenerHistorialPorUsuario(Integer idUsuario) {
                ...
                return reservas.stream().map(reserva -> {
                        ...
                        List<HabitacionResumen> habitaciones = reservaHabitacionRepository
                                        .buscarPorReserva(reserva.getId_reserva())
                                        .stream().map(rh -> new HabitacionResumen(
                                                        rh.getHabitacion().getNombre_habitacion(),
                                                        rh.getHabitacion().getCategoriaHabitacion()
                                                                        .getNombre_categoria(),
                        ...
                        List<ServicioResumen> servicios = reservaServicioRepository
                                        .buscarPorReserva(reserva.getId_reserva())
                        ...
                        pagoRepository.buscarPorReserva(reserva.getId_reserva()).ifPresent(pago -> {
                                PagoResumen pagoResumen = new PagoResumen(
                                                ...
                                                pago.getFecha_pago(),
                                                pago.getMetodoPago() != null ? pago.getMetodoPago().getTipo().name()
                                                                : null);
